	...
	for (int y = 0; y < image->h; ++y)
	{
	    for(int x = 0; x < image->w; ++x)
		{
		    ...
		    if (v1.size() >= 8)
            {
                ...
                v_temp.erase(v_temp.begin(), v_temp.begin()); // clean up already saved values
            }
		}
	}
...
Converter::DataVector Converter::unpacker(DataVector& vals)
{
    ...
    auto end_it = std::next(vals.begin(), 7);  // find next position after 8-th element
